...
module.exports = {
  ...
  run: async(client, message, args) => {
    ...
    if (!user) {
      ...
      .setDescription(`iunmute a user`)
      .addFields(
      { name: '**usage**', value: `${guildprefix}iunmute [user]`, inline: false },
    ...
    if (!user.roles.cache.has(muterole1?.id)) return message.channel.send('user is already iunmuted')

    user.roles.remove(muterole1.id, reason).then(() => {
      return message.channel.send(`**${user.user.tag}** has been image unmuted 👍`)
    }).catch(() => {
      return message.channel.send(`an error occured`)
    })
  }
