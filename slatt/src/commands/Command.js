...
class Command {
  ...
  checkUserPermissions(message, ownerOverride = true) {
    ...
    if (this.userPermissions) {
      ...
      if (missingPermissions.length !== 0) {
        const embed = new MessageEmbed().setDescription(`> <:anti:828540580136484884> ${message.author} \`Missing Permissions:\` You need the **${missingPermissions.join(", ")}** permission${missingPermissions.length > 1 ? 's' : ''} to run \`${this.name}\``).setColor("eea62c")
  ...
  checkClientPermissions(message) {
    ...
    if (missingPermissions.length !== 0) {
      const embed = new MessageEmbed().setDescription(`> <:anti:828540580136484884> ${message.author} \`Missing Permissions:\` I need the **${missingPermissions.join(", ")}** permission${missingPermissions.length > 1 ? 's' : ''} to run \`${this.name}\``).setColor("eea62c")
      message.channel.send({ embeds: [embed] })
      return false;
  ...
  async help(message) {
    ...
    const embed = new MessageEmbed()
      .setFooter(`Command Type: ${this.type} • Cooldown: ${this.cooldown || "None"} • do ${prefix}help ${this.name}`)
      .setTitle('Command')
      .setAuthor('Command: ' + prefix + this.name)
      .setDescription(`\`\`\`
Usage: ${prefix}${usage}
