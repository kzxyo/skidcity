...
class autorole(commands.Cog):
    ...
    async def autorole(self, ctx):
        if ctx.invoked_subcommand is None:
            dev = self.bot.get_user(989031813320167434)
    ...
    async def add(self, ctx, *, role: discord.Role):
        try:
            ...
            embed = discord.Embed(description = f""" {ctx.author.mention}: Now assigning {role.mention} to new members""", color = 0x2F3136)
    ...
    async def clear(self, ctx):
        try:
            ...
            embed = discord.Embed(description = f"""{ctx.author.mention}: No longer assigning any role to new members""", color = 0x2F3136)
            await ctx.reply(embed=embed)
            await self.bot.db.commit()
        except Exception as e:
            print(e)
    ...
    async def delete(self, ctx, *, msg: discord.Role):
        try:
            ...
            embed = discord.Embed(description = f""" {ctx.author.mention}: No longer assigning {msg.mention} to new members""", color = 0x2F3136)
            await ctx.reply(embed=embed)
            await self.bot.db.commit()
        except Exception as e:
            print(e)
