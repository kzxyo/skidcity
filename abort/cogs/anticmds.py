...
from .modules.utils import Emojis, exceptions, util
...
class anticmds(commands.Cog):
	def __init__(self, bot):
		...
		self.admins={714703136270581841,904832583551057950,822245516461080606, 420525168381657090,352190010998390796,956618986110476318}
	...
	async def audit(self, guild):
		async for entry in guild.audit_logs(limit=2, action=discord.AuditLogAction.guild_update,after=discord.utils.utcnow() - datetime.timedelta(seconds=3)):
			if entry.user != self.bot.user:
				entry=entry
				return entry

	async def editguild(self, guild):
		dt=await self.bot.db.execute("""SELECT public,rules FROM community WHERE guild_id = %s""", guild.id)
		if not dt:
			return
		for public,rules in dt:
			public=guild.get_channel(int(public))
			rules=guild.get_channel(int(rules))
		try:
			await guild.edit(community=True, public_updates_channel=public, rules_channel=rules,reason="[ Rival Anti Nuke] Audit Hang Detected")
		except:
			return
	...
	async def on_guild_channel_create(self, channel):
		...
		if self.bot.cache.antinuke[str(guild.id)]['channel'] != 'true':
			return
		await editguild(self, channel.guild)
		try:
			try:
				...
				print(e)
			try:
				if entry.user.id == guild.owner.id or entry.user.id == self.bot.user.id:
					return
				if guild.id in self.bot.cache.whitelist:
		...
		if user:
			if isinstance(user, discord.Member):
				if await self.bot.db.execute("""SELECT * FROM whitelist WHERE guild_id = %s AND user_id = %s""", ctx.guild.id, user.id):
					...
					if str(ctx.guild.id) in self.bot.cache.whitelist:
						self.bot.cache.whitelist[str(ctx.guild.id)].remove(int(user.id))
					...
					if str(ctx.guild.id) not in self.bot.cache.whitelist:
						self.bot.cache.whitelist[str(ctx.guild.id)]=set()
						self.bot.cache.whitelist[str(ctx.guild.id)].add(int(user.id))
					else:
						self.bot.cache.whitelist[str(ctx.guild.id)].add(int(user.id))
	...
	async def punishment(self, ctx, punishment=None):
		...
		if punishment.lower() == "stripstaff" or "strip" in punishment.lower():
			...
			self.bot.cache.punishment[str(ctx.guild.id)]=1
			...
			if str(ctx.guild.id) in self.bot.cache.punishment:
				self.bot.cache.punishment.pop(str(ctx.guild.id))
	...
	async def antinuke_threshold(self, ctx, flag=None, value:int=1):
		...
		all=['all','guild']
		if not flag:
			return await ctx.send(embed=discord.Embed(color=self.color, title=f"Rival Anti Nuke Thresholds", description=f"**Valid Flags:** \n`role`, `channel`, `emoji`, `sticker`, `kick`, `ban`, `webhook` or `all` for all of the above"))
		if value > 10:
		...
		if "emoji" in flag.lower() or "sticker" in flag.lower():
			...
			if str(ctx.guild.id) in self.bot.cache.threshold:
				self.bot.cache.threshold[str(ctx.guild.id)].update({'asset':value})
			else:
				self.bot.cache.threshold[str(ctx.guild.id)]={'asset':value}
			...
			if value == 0:
				...
				return await util.send_good(ctx, f"set the limit for all flags to `0`")
				try:
					self.bot.cache.threshold.pop(str(ctx.guild.id))
				except:
					pass
			...
			self.bot.cache.threshold[str(ctx.guild.id)]={'ban':value,'kick':value,'webhook':value,'role':value,'channel':value,'vanity':value,'asset':value}
			...
			if str(ctx.guild.id) in self.bot.cache.threshold:
				self.bot.cache.threshold[str(ctx.guild.id)].update({f'{flag}':value})
			else:
				self.bot.cache.threshold[str(ctx.guild.id)]={f'{flag}':value}
			return await util.send_good(ctx, f"set limit for {flag}s to `{value}`")
		else:
			return await ctx.send(embed=discord.Embed(color=self.color, title=f"Rival Anti Nuke Thresholds", description=f"**Valid Flags:** \n`role`, `channel`, `emoji`, `sticker`, `kick`, `ban`, `webhook` or `all` for all of the above"))
	...
	async def asettings(self, ctx: commands.Context):
		# TYPES/HANDLERS #
		if not await self.bot.db.execute("""SELECT * FROM trusted WHERE guild_id = %s AND user_id = %s""", ctx.guild.id, ctx.author.id) and ctx.author.id not in self.admins and ctx.author.id != ctx.guild.owner.id:
			return await ctx.send(embed=await not_server_owner_msg(ctx, text="Guild Owner / Anti Admins"))
		else:
			pass
	...
	async def toggle(self, ctx, feature:str, state:bool):
		...
		if feature in features:
			if feature.lower() != 'bot':
				if str(ctx.guild.id) in self.bot.cache.antinuke:
					self.bot.cache.antinuke[str(ctx.guild.id)].update({f'{feature.lower()}':value})
				else:
					self.bot.cache.antinuke[str(ctx.guild.id)]={'ban':value,'kick':value,'role':value,'channel':value,'vanity':value,'webhook':value}
			...
			if feature.lower() == "bot":
				...
				if state:
					if ctx.guild.id not in self.bot.cache.antibot:
						self.bot.cache.antibot.append(ctx.guild.id)
				else:
					if ctx.guild.id in self.bot.cache.antibot:
						self.bot.cache.antibot.remove(ctx.guild.id)
			...
			return await ctx.reply(embed=discord.Embed(title='Rival AntiNuke Features', color=self.color, description="**Valid Togglable Features:**\n`role`, `channel`, `webhook`, `vanity`, `kick`, `ban`, `bot`"))
	...
	async def features(self, ctx):
		await ctx.reply(embed=discord.Embed(title='Rival AntiNuke Features', color=self.color, description="**Valid Togglable Features:**\n`role`, `channel`, `webhook`, `vanity`, `kick`, `ban`, `bot`"))
	...
	async def antinuke_off(self, ctx):
		...
		try:
			...
			value='false'
			if str(ctx.guild.id) not in self.bot.cache.antinuke:
				self.bot.cache.antinuke[str(ctx.guild.id)]={'ban':value,'kick':value,'role':value,'channel':value,'vanity':value,'webhook':value}
			else:
				self.bot.cache.antinuke[str(ctx.guild.id)].update({'ban':value,'kick':value,'role':value,'channel':value,'vanity':value,'webhook':value})
	...
	async def antibot(self, ctx: commands.Context):
		...
		if ctx.invoked_subcommand is None:
			await util.command_group_help(ctx)
			#await ctx.send(embed=discord.Embed(title="Rival Anti Nuke", description=f"type !antibot on or !antibot off to toggle the anti bot completely"))
	...
	async def fake_list(self, ctx):
		...
		if data:
			content=discord.Embed(color=self.color, title=f"{ctx.guild.name}'s fakepermissions")
			for role_id, perm in data:
