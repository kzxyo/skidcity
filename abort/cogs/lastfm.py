            ...
            async with message.channel.typing():
              ...
              if res is not None:   
                 ...
                 if user != "error":   
                    ...
                    embed = discord.Embed(colour=Colors.default)
                ...
                embed = discord.Embed(description=f"<:lastfm:1059878318599569428> **{member}** doesn't have a **Last.fm account** linked. Use `,lf set <username>` to link your **account**.", color=Colors.red)
                await message.reply(embed=embed, mention_author=False)  
...
through the bot!""",color = Colors.default)
...
>   the other commands are displayed in the help command""", inline = False)
    ...
    async def customcommand(self, ctx, *, cmd=None):
        ...
        async with self.bot.db.cursor() as cursor:
                    ...
                    embed = discord.Embed(description=f'<:lastfm:1059878318599569428> {ctx.message.author.mention}: Your **Last.fm** custom command is {cmd}', color=Colors.red)
                    await sendmsg(self, ctx, None, embed, None, None, None)   
                    await self.bot.db.commit()  
...
`10` **[{topartist10}]({topartist10url})** {topartist10plays} plays""",color = Colors.default)
...
`10` **[{toptrack10}]({toptrack10url})** {toptrack10plays} plays""",color = Colors.default)
...
`10` **[{topalbum10}]({topalbum10url})** {topalbum10plays} plays""",color = Colors.default)
                        embed.set_thumbnail(url = ctx.message.author.avatar)
