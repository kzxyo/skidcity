...
import asyncio
import discord, typing, traceback
...
class Paginator(discord.ui.View):
    def __init__(
        ...
        *,
        ...
        super().__init__(timeout=None) # timeout=timeout
        self.check = check
        self.bot = bot
        self.defer = defer
        self.embeds = embeds
    ...
    def default_pagination(self, show_page: bool = False):
        self.add_button('first', emoji='<:v_first_page:1067034043901804564>')
        self.add_button('prev', emoji='<:v_previous_page:1067034040223420467>')

        if show_page is True:
            self.add_button('show')
            
        self.add_button('next', emoji='<:v_next_page:1067034038386303016>')
        self.add_button('last', emoji='<:v_last_page:1067034034946977792>')
        self.add_button('goto', emoji='<:v_page_skip:1067034020489220166>')
        ...
        if not self.invoker:
            ...
            if interaction.user.id != self.invoker:
                return await interaction.response.send_message(
                    ...
                    embed=discord.Embed(
                        color=0xB1AAD8,
                        description=f'<:v_warn:1067034029569888266> {interaction.user.mention}: you can\'t **interact** with this',
                    ),
                )
            else:
...
def embed_creator(text: str, num: int = 1980, /, *, title: str = '', prefix: str = '', suffix: str = '', color: int = 0xb1aad8):
