...
class Mod: 
  ...
  def is_mod_configured(): 
   async def predicate(ctx: commands.Context): 
    ...
    if not check: 
     await ctx.send_warning( f"Jail isn't **enabled** in this server. Enable it using `{ctx.clean_prefix}setme` command")
     return False
    return True
...
class Pot: 
  ...
  def check_pot():
   async def predicate(ctx: commands.Context): 
    ...
    if not check: await ctx.bot.ext.send_error(ctx, f"This server doesn't have a **pot**. Use `{ctx.clean_prefix}pot toggle` to get one")    
  ...
  def pot_owner(): 
   async def predicate(ctx: commands.Context): 
    check = await ctx.bot.db.fetchrow("SELECT * FROM pot WHERE guild_id = $1", ctx.guild.id)
    if check["holder"] != ctx.author.id: await ctx.send_warning( f"You don't have the **pot**. Steal it from <@{check['holder']}>")
