import asyncio, discord, textwrap, random, math, os,datetime
...
class Embed:
    def __init__(self, **kwargs):
        ...
        self.force_pm = kwargs.get("force_pm", False)
        self.pm_react = kwargs.get("pm_react", "📬")
        self.content = kwargs.get("content", None) or kwargs.get("message", None)
        self.message = self.content
        self.title = kwargs.get("title", None)
        self.timestamp = kwargs.get("timestamp", None)
        if self.timestamp:
            #ts=datetime.datetime.strptime(float(self.timestamp), '%Y-%m-%dT%H::%M::%S.%f')
            #self.timestamp=datetime.datetime.fromtimestamp(float(self.timestamp))
        ...
        if self.image:
            try:
                self.image={"url": kwargs.get("image", None)}
            except:
        ...
        if self.thumbnail:
            try:
                self.thumbnail={"url": kwargs.get("thumbnail", None)}
            except:
        ...
        self.file = kwargs.get("file", None) # Accepts a file path
        self.delete_after = kwargs.get("delete_after",None) or kwargs.get("delete",None) or kwargs.get("autodelete",None)
        if self.delete_after:
            self.delete_after=int(self.delete_after)
        self.colors = [ 
            discord.Color.teal(),
    ...
    async def _send_embed(self, ctx, embed, pm = False, file_path = None):
        ...
        if pm == True and type(ctx) is discord.ext.commands.Context and not ctx.channel == ctx.author.dm_channel:
            ...
            try:
                message = await ctx.author.send(content=self.content,embed=embed,file=send_file,delete_after=self.delete_after)
                await ctx.message.add_reaction(self.pm_react)
                return message
            except discord.Forbidden:
                if self.force_pm:
        ...
        if self.buttons:
            ...
            ran=len(self.buttons)
            for i in range(ran):
                try:
                    style=self.buttons[i].get("style")
                except:
                    style=None
                try:
                    emoji=self.buttons[i].get("emoji")
                except:
                    emoji=None
                try:
                    url=self.buttons[i].get("url")
                except:
                    url=None
                try:
                    label=self.buttons[i].get("label")
                except:
                    label=None
                view.add_item(discord.ui.Button(style=str(f"discord.ButtonStyle.{style}"), label=label, emoji=emoji, url=url))
        else:
            view=None

        return await ctx.send(content=self.content,embed=embed,file=send_file,delete_after=self.delete_after, view=view,allowed_mentions=discord.AllowedMentions(users=True, roles=True, everyone=False))
    ...
    def _embed_with_self(self):
        ...
        if self.image:
            em.set_image(url=self.image.get("url",None) if isinstance(self.image,dict) else self.image)
        if self.thumbnail:
            em.set_thumbnail(url=self.thumbnail.get("url",None) if isinstance(self.thumbnail,dict) else self.thumbnail)
        if self.author:
            if type(self.author) is discord.Member or type(self.author) is discord.User:
                ...
                if any(item in self.author for item in ["name", "url", "icon"]):
                    em.set_author(
                        name = self._truncate_string(self.author.get("name",     None), self.auth_max),
    ...
    def _get_footer(self):
        ...
        if type(self.footer) is str:
            ...
            footer_text = self.footer.get("text", None)
            footer_icon = self.footer.get("icon_url", None)
