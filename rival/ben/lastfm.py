...
class LastFm(commands.Cog):
	...
	@fm.command(name='embed', description="Set your custom NowPlaying Command Embed", extras={'perms': 'donator'}, brief='embedjson')
	@util.donor()
	async def embed(self, ctx, *, embed_json=None):
		if embed_json == None:
			return await ctx.reply(embed=discord.Embed(description=f"For Embed Creation use [This Embed Creator](https://rival.rocks/embedbuilder/)\n**Variables:**\n`$(track)`, `$(artist)`, `$(album)`, `$(playcount)`, `$(user)`, `$(avatar)`, `$(track.url)`, `$(artist.url)`, `$(tags)`, `$(scrobbles)`, `$(track.color)`, `$(track.image)`, `$(lastfm.user)`, `$(state)`"))
		if not embed_json:
			return await ctx.reply(embed=discord.Embed(description=f"For Embed Creation use [This Embed Creator](https://rival.rocks/embedbuilder/)\n**Variables:**\n`$(track)`, `$(artist)`, `$(album)`, `$(playcount)`, `$(user)`, `$(avatar)`, `$(track.url)`, `$(artist.url)`, `$(tags)`, `$(scrobbles)`, `$(track.color)`, `$(track.image)`, `$(lastfm.user)`, `$(state)`"))
...
#		author_artists = author_data["topartists"]["artist"]
		...
		for i,artist_name in enumerate(t,start=0):
			...
			space=" "
			spaces=" "*int(spaces_needed)
			dd.append(spaces_needed)
		...
		with contextlib.suppress(discord.NotFound):
			await ctx.send(content=dd, embed=embed)
	...
	async def userinfo(self, ctx, member: Union[discord.Member, discord.User] = None):
		...
		if member in ctx.guild.members: 
			...
			em.add_field(name=f"**Roles({len(member.roles[-1:0:-1])})**", value=f"{top_role}", inline=True)
