...
class Feed:
    ...
    async def for_you(self, count="1", max_cursor="1", min_cursor="1", region="US", raw_data=False):
        ...
        """
        try:
            pull_type = await self.set_pull_type(min_cursor, max_cursor, count)
            _rticket = str(time.time() * 1000).split(".")[0]
            ts = str(time.time()).split(".")[0]
            req_from = await self.set_req_from(min_cursor, max_cursor)
    ...
    async def trending_categories(self, cursor="0", count="10", region="US", raw=False):
        ...
        if raw:
            ...
            return await self.format_categories(res["category_list"])
                    
    async def set_pull_type(self, min_cursor, max_cursor, count):
        if max_cursor and min_cursor == "0":
            return '4'
        if max_cursor == "0" and min_cursor != "0":
            return '2'
        
    async def set_req_from(self, min_cursor, max_cursor):
        if min_cursor != "0" and max_cursor == "0":
            return 'req_from'
        else:
            return 'req_from=enter_auto'
    
    async def format_categories(self, cat_data) -> list:
        data = []
        for cat in cat_data:
            t = cat["category_type"]
            category = {}
            category["trending_type"] = await self.trending_types[t]
            if t == 1:
            ...
            for video in cat["aweme_list"]:
                category["videos"].append(await self.format_cat_video(video))
            data.append(category)
        return data
            
    async def format_cat_video(self, data):
        cat_data = {}
