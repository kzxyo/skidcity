import discord, requests, platform, animec, asyncio, datetime, humanize, random
...
class socials(commands.Cog):
    ...
    @commands.command(name = "youtube", description = "Show info about youtube profile",aliases=["yt", "tube", "you"], usage = "youtube [user]")
    ...
    async def youtube(self, ctx, *, username):
        ...
        if not search_response["items"]:
            embed = discord.Embed(
                title="<:mirodeny:1117144156507209829> Error",
        ...
        embed = discord.Embed(
            title=f"<:youtube:1113553655002562651> {title}", color=0x4c5264
        )
        user = ctx.author
        ...
        button = YoutubeButton(
            channel_id, custom_emoji="<:youtube:1113553655002562651>"
        )
    ...
    @commands.command(name = "tiktok", description = "Show info about tiktok profile",aliases=["tic", "tik", "tok", "toc", "tt"], usage = "tiktok [user]")
    ...
    async def fetch_tiktok_profile(self, ctx, username):
        ...
        embed.add_field(
            name="<:user:1113553510211010712> Username",
            value=f"> *@{username}*",
            inline=False,
        )
        embed.add_field(
            name="<:Recording:1114291307494526976> Posts",
            value=f"> `{videos}`",
            inline=False,
        )
        embed.add_field(
        ...
        embed.add_field(
            name="<:___follow:1113553548857323620> Followers",
            value=f"> `{followers}`",
            inline=False,
        )
        embed.add_field(
            name="<:___follow:1113553548857323620> Following",
            value=f"> `{following}`",
            inline=False,
        )
        embed.add_field(
            name="<:description:1114291768788267148> Bio",
            value=f"> *{bio}*",
            inline=False,
        )
    ...
    @commands.command(description="Shows information on an anime", usage="anime <name>", aliases=["ani"])
    @commands.cooldown(1, 4, commands.BucketType.guild)
    ...
    async def reminder(self, ctx, time, *, reminder):
        try:
            ...
            if seconds == 0:
                ...
                embed = discord.Embed(
                    description=f"{ctx.message.author.mention}:<:miroapprove:1117144152363245638> I will remind you about `{reminder}` in {counter}",
                    color=0x4c5264,
                )
                await ctx.reply(embed=embed)
