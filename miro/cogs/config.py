import discord, datetime, typing, traceback, re
...
class config(commands.Cog):
    ...
    async def guildprefix(self, ctx, stat: str = None, prefix=None):
        try:
            ...
            note2.add_field(
                name="Usage",
                value=f"{self.reply} **syntax:** ;guildprefix set <prefix>\n{self.reply} **example:** ,guildprefix set Miro",
                inline=False,
            )
            note3 = discord.Embed(color=0x4c5264, timestamp=datetime.now())
            ...
            paginator = pg.Paginator(self.bot, [note, note2, note3], ctx, timeout=50, invoker=ctx.author.id)
            ...
            match stat:
                case "set":
                    ...
                    await ctx.reply(
                        embed=discord.Embed(
                            color=self.success,
                            description=f"{utils.read_json('emojis')['done']} {ctx.author.mention}**:** the prefix has been set to **`{['prefix']}`**",
                        )
                    )
    ...
    async def autor_delete(self, ctx, trigger: str = None):
        try:
            ...
            note2.add_field(
                name="Usage",
                value=f"{self.reply} **syntax:** ;autoresponder add <trigger> <response>\n{self.reply} **example:** ;autoresponder add Miro hot bot",
                inline=False,
            )
            note3 = discord.Embed(color=0x4c5264, timestamp=datetime.now())
            note3.set_author(name="autoresponder delete", icon_url=self.bot.user.avatar)
    ...
    async def autor_list(self, ctx):
        ...
        paginator = pg.Paginator(self.bot, embeds, ctx, invoker=ctx.author.id, timeout=50)
        paginator.add_button("prev", emoji="<:left:1107307769582850079>")
        paginator.add_button("goto", emoji="<:filter:1113850464832868433>")
        paginator.add_button("next", emoji="<:right:1107307767041105920>")
        paginator.add_button("delete", emoji="<:page_cancel:1121826948520362045>")
    ...
    @commands.hybrid_command(name = "ignorechannel", description = "Ignore a channel from commands",aliases=['ic'], usage = "ignorechannel <channel>")
    @commands.cooldown(1, 4, commands.BucketType.guild)
    @utils.perms("manage_guild")
    ...
    @commands.hybrid_command(name = "unignorechannel", description = "Unignore a channel",aliases=['uic'], usage = "unignorechannel <channel>")
    @commands.cooldown(1, 4, commands.BucketType.guild)
    @utils.perms("manage_guild")
    ...
    async def boost(self, ctx, *, msg: str = None):
        ...
        if not msg:
            ...
            note4 = discord.Embed(color=0x4c5264, timestamp=datetime.now())
            ...
            note5 = discord.Embed(color=0x4c5264, timestamp=datetime.now())
        ...
        try:
            if not msg:
                ...
                e.add_field(
                    ...
                    value=f"{self.reply} **description:** set up the guild's mod-logs channel\n{self.reply} **aliases:** setmodlogs, sml, modlogs\n{self.reply} **extra:** setting it to 'none' removes the ban message",
                    inline=False,
                )
                e.add_field(
            ...
            paginator = pg.Paginator(self.bot, [e, e2], ctx, invoker=ctx.author.id, timeout=50)
            paginator.add_button("prev", emoji="<:left:1107307769582850079>")
            paginator.add_button("goto", emoji="<:filter:1113850464832868433>")
            paginator.add_button("next", emoji="<:right:1107307767041105920>")
            paginator.add_button("delete", emoji="<:page_cancel:1121826948520362045>")
    ...
    async def pingonjoin(self, ctx):
        ...
        paginator = pg.Paginator(self.bot, [note, note2, note3], ctx, invoker=ctx.author.id, timeout=50)
        paginator.add_button("prev", emoji="<:left:1107307769582850079>")
        paginator.add_button("goto", emoji="<:filter:1113850464832868433>")
        paginator.add_button("next", emoji="<:right:1107307767041105920>")
        paginator.add_button("delete", emoji="<:page_cancel:1121826948520362045>")
    ...
    async def antiinvite(self, ctx):
        ...
        paginator = pg.Paginator(self.bot, [note, note2], ctx, invoker=ctx.author.id, timeout=50)
        paginator.add_button("prev", emoji="<:left:1107307769582850079>")
        paginator.add_button("goto", emoji="<:filter:1113850464832868433>")
        paginator.add_button("next", emoji="<:right:1107307767041105920>")
        paginator.add_button("delete", emoji="<:page_cancel:1121826948520362045>")
    ...
    @commands.cooldown(1, 4, commands.BucketType.guild)
    @utils.perms("manage_guild")
    async def disable2(self, ctx, *, command: str):
    ...
    @commands.cooldown(1, 4, commands.BucketType.guild)
    @utils.perms("manage_guild")
    async def enable2(self, ctx, *, command: str):
        if not self.bot.get_command(command):
            await ctx.reply(
                embed=discord.Embed(
                    color=self.warning,
    ...
    @utils.perms("manage_guild")
    async def settings2(self, ctx):
        ...

    @settings2.command(name="safesearch", aliases=["ss", "gs"])
    @commands.cooldown(1, 4, commands.BucketType.guild)
    ...
    @settings2.command(name="disable", aliases=["dis"])
    @utils.perms("manage_guild")
    ...
    @settings2.command(name="enable", aliases=["en"])
    @utils.perms("manage_guild")
    async def settings2_enable(self, ctx, *, command: str):
        if not self.bot.get_command(command):
            await ctx.reply(
                embed=discord.Embed(
                    color=self.warning,
    ...
    @settings2.command(name="antiinvite-on")
    @utils.perms("manage_guild")
    ...
    @settings2.command(name="antiinvite-off")
    @utils.perms("manage_guild")
