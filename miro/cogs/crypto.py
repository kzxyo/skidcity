...
class crypto(commands.Cog):
    ...
    # embed = discord.Embed(color=0x4c5264, title=f"Current {name} Value", description=f"1 Single {name}", timestamp=datetime.utcnow())
    # embed.add_field(name="USD", value=f"{usd:,}", inline=True)
    # embed.add_field(name="GBP", value=f"{gbp:,}", inline=True)
    ##embed.add_field(name="EUR", value=f"{eur:,}", inline=True)
    # embed.set_footer(text="Miro", icon_url='https://cdn.discordapp.com/attachments/1116443743831199814/1118245305352200332/Cherry_Blossoms.png')
    # embed.set_thumbnail(url="https://pngimg.com/uploads/bitcoin/bitcoin_PNG48.png")
    # await ctx.reply(embed=embed)
