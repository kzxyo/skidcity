...
class mod(commands.Cog):
    ...
    if subcommand == "remove":
      ...
      if target == "humans":
        ...
        try:
         ...
         await message.edit(embed=discord.Embed(color=Colors.green, description=f"{Emojis.remove} {ctx.author.mention}: removed {role.mention} from all {target}"))
        ...
        embed = discord.Embed(color=Colors.default, description=f"{ctx.author.mention} removing {role.mention} from all members....")
        message = await ctx.reply(embed=embed, mention_author=False)
        try:
         for member in ctx.guild.members: 
            if not role in member.roles: continue
      ...
      if target == "humans":
        ...
        try:
         ...
         await message.edit(embed=discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: added {role.mention} to all {target}"))
        except Exception as e:
        ...
        try:
         ...
         await message.edit(embed=discord.Embed(color=Colors.green, description=f"{Emojis.add} {ctx.author.mention}: added {role.mention} from all {target}"))
        except Exception as e:
          await message.edit(embed=discord.Embed(color=Colors.red, description=f"{Emojis.wrong} {ctx.author.mention}: unable to add {role.mention} to all {target} - {e}"))    
      elif target == "all": 
        ...
        try:
         ...
         await message.edit(embed=discord.Embed(color=Colors.green, description=f"{Emojis.add} {ctx.author.mention}: added {role.mention} from all"))
        except Exception as e:
   ...
   async def ban(self, ctx, member: Union[discord.Member, discord.User]=None, *, reason=None):
        ...
        if reason:
            reason=f'{reason}'
        if member is None:
            embed = discord.Embed(color=Colors.default, title=f'ban', description='ban a member')
            embed.set_author(name=f"{self.bot.user.name}", icon_url=self.bot.user.display_avatar)
            embed.add_field(name='category', value='moderation', inline=False)
            embed.add_field(name="permissions", value="ban_members", inline=True)
            embed.add_field(name='uasge', value='```;ban [member] <reason>```', inline=False)
            embed.add_field(name='example', value='```;ban @mozzy#0001 egoing```', inline=True)
            embed.add_field(name='aliases', value='yeet', inline=False)
        ...
        if user.count('#') == 1:
            ...
            async for ban in ctx.guild.bans():
                if (ban.user.name, ban.user.discriminator) == (username, discriminator):
                    ...
                    await ctx.send(f'👍🏾', mention_author=False)
                    return
        else:
            try:
                user_id = int(user)
   ...
   async def kick(self, ctx, user: discord.Member, *, reason=None):
        ...
        if ctx.author.top_role.position <= user.top_role.position and ctx.author != ctx.guild.owner:
            ...
            e2 = discord.Embed(title = "kicked", color = Colors.red)
            e2.add_field(name="server", value=ctx.guild, inline=True)
            e2.add_field(name="admin", value=ctx.author, inline=True)
            e2.add_field(name="reason", value=reason, inline=True)
   ...
   async def role(self, ctx, user : discord.Member=None, *, role : discord.Role=None):
    ...
    if role in user.roles:
      ...
      embed = discord.Embed(color=Colors.green, description=f"<:remove:1108885089846300774> {ctx.author.mention}: removed {role.mention} from {user.mention}") 
      await sendmsg(self, ctx, None, embed, None, None, None)
    else:
      await user.add_roles(role)
      emb = discord.Embed(color=Colors.green, description=f"<:add:1108885082372051006> {ctx.author.mention}: given {role.mention} to {user.mention}") 
