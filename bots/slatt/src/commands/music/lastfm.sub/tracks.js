...
module.exports = class Soundcloud extends Subcommand {
    constructor(client) {
        super(client, {
            ...
            usage: 'lastfm tracks [user]',
            description: 'View a list of your last.fm tracks',
        });
    }
    async run(message, args) {
        ...
        var url = "http://ws.audioscrobbler.com/2.0/?method=user.gettoptracks&user=" + user.username + "&api_key=" + this.config.LASTFMKEY + "&format=json&limit=100";
        var options = {
            uri: url,
            json: true
        };
        ...
        rp(options)
            .then(function (tracks) {
                let num = 1
                let list = tracks.toptracks.track.map(t => `\`${num++}\` **[${t.name}](${t.url})** with **${t.playcount}** plays`)
                if (!list.length) {
                    const embed = new MessageEmbed()
                        .setDescription(`<:info:828536926603837441> ${message.author} You dont have any **Top Tracks**`)
                        .setColor("#78c6fe");
                    return message.channel.send({ embeds: [embed] });
                }
                let profile_image = userinfo.user.image[3]['#text']
                const embed = new MessageEmbed()
                    .setTitle(`${user.username}'s Tracks`)
                    ...
                    .setFooter(`Top tracks: ${list.length}`, message.author.avatarURL({
                        dynamic: true
                    }))
                if (list.length <= 10) {
