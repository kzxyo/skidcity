...
class Mod(commands.Cog):
	...
	def __init__(self, bot):
		...
		self.good=self.bot.color#0xa5eb78
	...
	async def unban(self, ctx, user: typing.Union[discord.User, int, str], *, reason:str=None):
		...
		try:
			...
			embed = discord.Embed(description=f"{self.bot.yes} {ctx.author.mention}: **{user_str}** has been unbanned", color=self.good)
			await ctx.send(embed=embed)
		except:
			embed=discord.Embed(description=f"{self.bot.warn} {ctx.author.mention}: **User Not Found**", color=self.error)
		...
		if isinstance(user, str):
			...
			if "#" in user:
				...
				if not banned_user:
					return await util.send_error(ctx, f"user `{user}` **not found** in bans")
			else:
				try:
					banned_user = discord.utils.get(
					bans, user__name=user
					)
				except:
					embed=discord.Embed(description=f"{self.bot.warn} {ctx.author.mention}: *Please format the username like this: \nUsername#0000*", color=self.error)
					await ctx.send(embed=embed)
		...
		if not isinstance(user, str):
			try:
				...
				embed = discord.Embed(description=f"{self.bot.yes} {ctx.author.mention}: **{user.name}#{user.discriminator}** has been unbanned", color=self.good)
				await ctx.send(embed=embed)
			except:
				embed=discord.Embed(description="{self.bot.warn} **User Not Found**", color=self.error)
				return await ctx.send(embed=embed)
	...
	@commands.hybrid_command(name='stfu', with_app_command=True, description="make a member's messages auto delete", usage="```Swift\nSyntax: !stfu <mention/id>\nExample: !stfu @cop#0001```",extras={'perms': 'administrator / donator server'}, brief='member')
	...
	async def stfu(self, ctx, member:discord.Member):
		...
		if await self.bot.db.execute("""SELECT * FROM stfu WHERE guild_id = %s and user_id = %s""", ctx.guild.id, member.id):
			...
			await util.send_success(ctx, f"{ctx.author.mention}: **unsilenced** {member.mention}")
		else:
			await self.bot.db.execute("""INSERT INTO stfu VALUES(%s, %s)""", ctx.guild.id, member.id)
			await util.send_success(ctx, f"{ctx.author.mention}: **silenced** {member.mention}")
