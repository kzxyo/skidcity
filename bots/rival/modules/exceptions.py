...
class EmbedFailure(commands.CommandError):
	def __init__(self, message):
		super().__init__(message)
		
	def display(self):
		return f"Embed Format Error Missing "+"`}`"+f" At position `{message}"
...
class BlacklistedGuild(Blacklist):
	def __init__(self):
		super().__init__()
		self.message = "This server is blacklisted from using Rival"
