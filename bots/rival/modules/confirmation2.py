...
async def confirm(self:discord.ext.commands.Cog, ctx:discord.ext.commands.Context, msg:discord.Message, invoker:discord.User=None, invoked:discord.User=None, timeout:int=180):
    ...
    if invoked == None:
        invoked=ctx.author
    view = Confirm(invoker=invoker, invoked=invoked, timeout=300)
...
class Confirm(discord.ui.View):
    ...
    @discord.ui.button(style=ButtonStyle.green, label="Yes")
    async def confirm(self, interaction:discord.Interaction, button:discord.ui.Button):
        await self.confirmation(interaction, True)

    @discord.ui.button(style=ButtonStyle.red, label="No")
    ...
    async def confirmation(self, interaction:discord.Interaction, confirm:bool):
        if confirm:
            if not interaction.user.id == self.invoker.id:
                await interaction.response.send_message(embed=discord.Embed(description=f"<:warning:1021286736883621959> {interaction.user.mention}: **You cannot interact with this**", color=int("faa61a", 16)), ephemeral=True)
                return
        else:
            if not interaction.user.id == self.invoker.id:
