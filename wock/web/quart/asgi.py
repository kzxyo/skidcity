...
from typing import TYPE_CHECKING, AnyStr, List, Optional, Set, Union, cast
...
class ASGIHTTPConnection:
    ...
    async def __call__(self, receive: ASGIReceiveCallable, send: ASGISendCallable) -> None:
        ...
        done, pending = await asyncio.wait([handler_task, receiver_task], return_when=asyncio.FIRST_COMPLETED)
    ...
    async def _send_response(self, send: ASGISendCallable, response: Union[Response, WerkzeugResponse]) -> None:
        await send(
            cast(
                HTTPResponseStartEvent,
                {
...
class ASGIWebsocketConnection:
    ...
    async def accept_connection(self, send: ASGISendCallable, headers: Headers, subprotocol: Optional[str]) -> None:
