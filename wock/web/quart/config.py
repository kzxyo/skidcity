...
class Config(dict):
    ...
    def from_envvar(self, variable_name: str, silent: bool = False) -> bool:
        ...
        if value is None:
            if silent:
                return False
            raise RuntimeError(f"Environment variable {variable_name} is not present, cannot load config")
        return self.from_pyfile(value)

    def from_prefixed_env(self, prefix: str = "QUART", *, loads: Callable[[str], Any] = json.loads) -> bool:
