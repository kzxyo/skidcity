...
class FileBody(ResponseBody):
    ...
    def __init__(self, file_path: Union[str, PathLike], *, buffer_size: Optional[int] = None) -> None:
...
class Response(SansIOResponse):
    ...
    async def _process_range_request(
        ...
        if accept_ranges is None or complete_length is None or complete_length == 0 or not self._is_range_request_processable(request):
            return False
