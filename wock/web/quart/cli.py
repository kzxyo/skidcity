...
def find_best_app(module: ModuleType) -> Quart:
    ...
    if len(matches) == 1:
        ...
        raise NoAppException(
            f"Detected multiple Quart applications in module '{module.__name__}'. Use '{module.__name__}:name' to specify the correct one."
        )
    ...
    raise NoAppException(f"Failed to find Quart application or factory in module '{module.__name__}'. Use '{module.__name__}:name' to specify one.")
...
def find_app_by_string(module: ModuleType, app_name: str) -> Quart:
    ...
    raise NoAppException(f"A valid Quart application was not obtained from '{module.__name__}:{app_name}'.")
            ...
            async with __ctx.ensure_object(ScriptInfo).load_app().app_context():
                try:
                    ...
                    if error.args[0] == "Cannot run the event loop while another loop is running":
                        click.echo("The appcontext cannot be used with a command that runs an event loop. See quart#361 for more details")
                    raise
...
def load_dotenv(path: str | os.PathLike | None = None) -> bool:
    ...
    try:
        ...
        if path or os.path.isfile(".env") or os.path.isfile(".quartenv"):
            click.secho(
                ' * Tip: There are .env or .quartenv files present. Do "pip install python-dotenv" to use them.',
                fg="yellow",
                err=True,
            )
...
def shell_command() -> None:
    ...
    """
    banner = f"Python {sys.version} on {sys.platform}\nApp: {current_app.import_name} [{current_app.env}]\nInstance: {current_app.instance_path}"
    ctx: dict = {}
...
@click.option(
    ...
    default="endpoint",
    help='Method to sort routes by. "match" is the order that Quart will match routes when dispatching a request.',
)
