...
class SessionInterface:
    ...
    async def open_session(self, app: "Quart", request: BaseRequestWebsocket) -> Optional[SessionMixin]:
...
class SecureCookieSessionInterface(SessionInterface):
    ...
    This will store the data on the cookie in pwock text, but with a
    ...
    async def open_session(self, app: "Quart", request: BaseRequestWebsocket) -> Optional[SecureCookieSession]:
        ...
        if response is None:
            if session.modified:
                app.logger.exception(
                    "Secure Cookie Session modified during websocket handling. These modifications will be lost as a cookie cannot be set."
                )
            return
