...
class Scaffold:
    ...
    def __init__(
        ...
        self.after_request_funcs: Dict[AppOrBlueprintKey, List[AfterRequestCallable]] = defaultdict(list)

        # Functions that are called after a WebSocket view function
        # handled a websocket request and has returned (possibly
        # returning a response).
        self.after_websocket_funcs: Dict[AppOrBlueprintKey, List[AfterWebsocketCallable]] = defaultdict(list)

        # Called before a HTTP view function handles a request.
        self.before_request_funcs: Dict[AppOrBlueprintKey, List[BeforeRequestCallable]] = defaultdict(list)

        # Called before a WebSocket view function handles a websocket
        # request.
        self.before_websocket_funcs: Dict[AppOrBlueprintKey, List[BeforeWebsocketCallable]] = defaultdict(list)
        ...
        self.teardown_request_funcs: Dict[AppOrBlueprintKey, List[TeardownCallable]] = defaultdict(list)

        # Called after a WebSocket request has been handled, even if
        # the handling results in an exception.
        self.teardown_websocket_funcs: Dict[AppOrBlueprintKey, List[TeardownCallable]] = defaultdict(list)

        # Template context processors keyed by blueprint (None for
        # app).
        self.template_context_processors: Dict[AppOrBlueprintKey, List[TemplateContextProcessorCallable]] = defaultdict(
        ...
        self.url_default_functions: Dict[AppOrBlueprintKey, List[URLDefaultCallable]] = defaultdict(list)
    ...
    @setupmethod
    def errorhandler(self, error: Union[Type[Exception], int]) -> Callable[[T_error_handler], T_error_handler]:
    ...
    def register_error_handler(
        ...
        if isinstance(error, HTTPException):
            raise ValueError("error must be an exception Type or int, not an instance of an exception")
    ...
    def _get_error_type_and_code(self, error: Union[Type[Exception], int]) -> Tuple[Type[Exception], Optional[int]]:
