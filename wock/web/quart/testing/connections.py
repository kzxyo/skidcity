...
from typing import TYPE_CHECKING, Any, AnyStr, Awaitable, List, Optional, Tuple
...
class TestHTTPConnection:
    ...
    async def __aenter__(self) -> "TestHTTPConnection":
        self._task = asyncio.ensure_future(self.app(self.scope, self._asgi_receive, self._asgi_send))
        return self
        ...
        if message["type"] == "websocket.accept":
            ...
            if not message.get("more_body", False):
                await self._receive_queue.put(
                    WebsocketResponseError(self.app.response_class(bytes(self.response_data), self.status_code, self.headers))
                )
