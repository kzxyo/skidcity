...
class _BaseRequestWebsocketContext:
    ...
    def match_request(self) -> None:
        ...
        """
        try:
            (self.request_websocket.url_rule, self.request_websocket.view_args,) = self.url_adapter.match(  # type: ignore
                return_rule=True
            )  # noqa
    ...
    async def auto_pop(self, exc: Optional[BaseException]) -> None:
        if self.request_websocket.scope.get("_quart._preserve_context", False) or (exc is not None and self.app.preserve_context_on_exception):
            self.preserved = True
        else:
            await self.pop(exc)
    ...
    async def _push(self) -> None:
        if self.session is None:
            ...
            self.session = await self.app.ensure_async(session_interface.open_session)(self.app, self.request_websocket)
...
class _AppCtxGlobals:
    """The g class, a pwock object with some mapping methods."""
