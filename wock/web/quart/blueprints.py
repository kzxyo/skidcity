...
T_template_context_processor = TypeVar("T_template_context_processor", bound=TemplateContextProcessorCallable)
...
class Blueprint(Scaffold):
    ...
    @setupmethod
    def app_template_filter(self, name: Optional[str] = None) -> Callable[[T_template_filter], T_template_filter]:
    ...
    @setupmethod
    def add_app_template_filter(self, func: TemplateFilterCallable, name: Optional[str] = None) -> None:
        """Add an application wide template filter.
    ...
    @setupmethod
    def app_template_test(self, name: Optional[str] = None) -> Callable[[T_template_test], T_template_test]:
    ...
    @setupmethod
    def app_template_global(self, name: Optional[str] = None) -> Callable[[T_template_global], T_template_global]:
    ...
    @setupmethod
    def add_app_template_global(self, func: TemplateGlobalCallable, name: Optional[str] = None) -> None:
        """Add an application wide template global.
    ...
    @setupmethod
    def app_errorhandler(self, error: Union[Type[Exception], int]) -> Callable[[T_error_handler], T_error_handler]:
    ...
    @setupmethod
    def app_url_value_preprocessor(self, func: T_url_value_preprocessor) -> T_url_value_preprocessor:
    ...
    def register(self, app: "Quart", options: dict) -> None:
        ...
        if name in app.blueprints and app.blueprints[name] is not self:
            raise ValueError(f"Blueprint name '{self.name}' is already registered by {app.blueprints[self.name]}. Blueprints must have unique names")

        first_blueprint_registration = not any(blueprint is self for blueprint in app.blueprints.values())
        ...
        if first_blueprint_registration or first_name_registration:
            ...
            for key, value in self.error_handler_spec.items():
                ...
                value = defaultdict(
                    dict,
                    {code: {exc_class: func for exc_class, func in code_values.items()} for code, code_values in value.items()},
                )
    ...
    def make_setup_state(self, app: "Quart", options: dict, first_registration: bool = False) -> "BlueprintSetupState":
