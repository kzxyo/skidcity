...
from .config import DEFAULT_CONFIG, Config, ConfigAttribute
from .ctx import (
    AppContext,
    RequestContext,
    WebsocketContext,
...
from .scaffold import Scaffold, _endpoint_from_view_func, setupmethod
...
T_shell_context_processor = TypeVar("T_shell_context_processor", bound=ShellContextProcessorCallable)
...
class Quart(Scaffold):
    ...
    permanent_session_lifetime = ConfigAttribute("PERMANENT_SESSION_LIFETIME", converter=_convert_timedelta)
    request_class = Request
    response_class = Response
    secret_key = ConfigAttribute("SECRET_KEY")
    send_file_max_age_default = ConfigAttribute("SEND_FILE_MAX_AGE_DEFAULT", converter=_convert_timedelta)
    ...
    def __init__(
        ...
        if self.has_static_folder:
            if bool(static_host) != host_matching:
                raise ValueError("static_host must be set if there is a static folder and host_matching is enabled")
            self.add_url_rule(
    ...
    def make_config(self, instance_relative: bool = False) -> Config:
        ...
        config = self.config_class(self.instance_path if instance_relative else self.root_path, DEFAULT_CONFIG)
        config["ENV"] = get_env()
    ...
    async def open_instance_resource(self, path: FilePath, mode: str = "rb") -> AiofilesContextManager[None, None, AsyncBufferedReader]:
    ...
    @setupmethod
    def template_filter(self, name: Optional[str] = None) -> Callable[[T_template_filter], T_template_filter]:
    ...
    @setupmethod
    def template_test(self, name: Optional[str] = None) -> Callable[[T_template_test], T_template_test]:
        """Add a template test.
    ...
    @setupmethod
    def template_global(self, name: Optional[str] = None) -> Callable[[T_template_global], T_template_global]:
    ...
    def url_for(
        ...
        **values: Any,
        ...
        if url_adapter is None:
            raise RuntimeError("Unable to create a url adapter, try setting the SERVER_NAME config variable.")
    ...
    async def handle_http_exception(self, error: HTTPException) -> Union[HTTPException, ResponseReturnValue]:
    ...
    async def handle_user_exception(self, error: Exception) -> Union[HTTPException, ResponseReturnValue]:
    ...
    async def handle_websocket_exception(self, error: Exception) -> Optional[Union[Response, WerkzeugResponse]]:
        """Handle an uncaught exception.
    ...
    async def make_response(self, result: Union[ResponseReturnValue, HTTPException]) -> Union[Response, WerkzeugResponse]:
        ...
        if isinstance(value, HTTPException):
            ...
            if isinstance(value, (str, bytes, bytearray)) or isgenerator(value) or isasyncgen(value):
    ...
    async def full_dispatch_request(self, request_context: Optional[RequestContext] = None) -> Union[Response, WerkzeugResponse]:
    ...
    async def preprocess_request(self, request_context: Optional[RequestContext] = None) -> Optional[ResponseReturnValue]:
    ...
    async def dispatch_request(self, request_context: Optional[RequestContext] = None) -> ResponseReturnValue:
    ...
    async def handle_websocket(self, websocket: Websocket) -> Optional[Union[Response, WerkzeugResponse]]:
    ...
    async def full_dispatch_websocket(self, websocket_context: Optional[WebsocketContext] = None) -> Optional[Union[Response, WerkzeugResponse]]:
    ...
    async def preprocess_websocket(self, websocket_context: Optional[WebsocketContext] = None) -> Optional[ResponseReturnValue]:
    ...
    async def dispatch_websocket(self, websocket_context: Optional[WebsocketContext] = None) -> Optional[ResponseReturnValue]:
    ...
    async def asgi_app(self, scope: Scope, receive: ASGIReceiveCallable, send: ASGISendCallable) -> None:
