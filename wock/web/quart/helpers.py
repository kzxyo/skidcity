...
async def flash(message: str, category: str = "message") -> None:
    ...
    await message_flashed.send(current_app._get_current_object(), message=message, category=category)  # type: ignore


def get_flashed_messages(with_categories: bool = False, category_filter: Iterable[str] = ()) -> Union[List[str], List[Tuple[str, str]]]:
...
async def send_file(
    ...
    if mimetype is None:
        raise ValueError("The mime type cannot be inferred, please set it manually via the mimetype argument.")
