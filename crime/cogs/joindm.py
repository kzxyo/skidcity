...
class joindm(commands.Cog):
    ...
    async def joindm(self, ctx):
        embed = discord.Embed(color=0x2f3136, title="joindm", description="greet your users into your guild with a direct message")
    ...
    async def test(self, ctx):   
        ...
        async with self.bot.db.cursor() as cursor:
            ...
            if check is not None:
                ...
                embed=discord.Embed(description=f"{Emojis.warn} joindm message isnt configured for *{ctx.guild.name}*", color=Colors.default)
                await ctx.reply(embed=embed, mention_author=False)
