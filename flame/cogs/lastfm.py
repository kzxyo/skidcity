...
through the bot!""",color = 0xffffff)
...
>   the other commands are displayed in the help command""", inline = False)
    ...
    async def set(self, ctx, ref=None):
            if ref is None: 
                await helpembed(self, ctx, "lastfm set")
                return 

            ref = ref.replace("https://www.last.fm/user/", "")
            if not await userhandler.lastfm_user_exists(ref):
                embed = discord.Embed(description=f'<:d_lastfm:1026538796709335090> **Invalid** Last.fm username',
                                    color=Colors.red)
                ...
                async with self.bot.db.cursor() as cursor:
                    ...
                    embed = discord.Embed(description=f'<:d_lastfm:1026538796709335090> {ctx.message.author.mention}: Your **Last.fm** username has been set to **{ref}**', color=Colors.red)
                    await ctx.reply(embed=embed)
                await self.bot.db.commit()  
    ...
    async def remove(self, ctx): 
     async with self.bot.db.cursor() as cursor:
        await cursor.execute("SELECT * FROM lastfm WHERE user_id = {}".format(ctx.author.id))
        check = await cursor.fetchone()
        if check is None: return await ctx.reply(embed=discord.Embed(description=f"<:d_lastfm:1026538796709335090> You don't have a **Last.fm account** linked. Use `,lf set <username>` to link your **account**.", color=Colors.red), mention_author=False)              
...
`10` **[{topartist10}]({topartist10url})** {topartist10plays} plays""",color = 0xffffff)
                        embed.set_thumbnail(url = member.display_avatar)
                        embed.set_author(name = f"{user}'s overall top artists", icon_url = member.display_avatar)
                        await ctx.reply(embed=embed)
            else:
                embed = discord.Embed(description=f"<:d_lastfm:1026538796709335090> You don't have a **Last.fm account** linked. Use `,lf set <username>` to link your **account**.",color=Colors.red)
...
`10` **[{toptrack10}]({toptrack10url})** {toptrack10plays} plays""",color = 0xffffff)
...
`10` **[{topalbum10}]({topalbum10url})** {topalbum10plays} plays""",color = 0xffffff)
                        embed.set_thumbnail(url = ctx.message.author.avatar)
      ...
      async with ctx.typing():
       ...
       for l in list:
        ...
        if listenerscount == 10:
         ...
         number.append(discord.Embed(color=0xffffff , description=messages[i]).set_author(name = f"Who knows {artist}?", icon_url = ctx.author.display_avatar).set_footer(text = f"{listenerscount} listeners for this artist"))
         i+=1
         listeners = ""
       if num == 0:
         await ctx.reply(f"Nobody (not even you) has listened to {artist}") 
         return 
       messages.append(listeners)
       embe = discord.Embed(color=0xffffff , description=messages[i]).set_author(name = f"Who knows {artist}?", icon_url = ctx.author.display_avatar).set_footer(text = f"{listenerscount} listeners for this artist")
       number.append(embe)
       if len(number) > 1:
