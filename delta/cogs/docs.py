...
class Docs(commands.Cog, name="Documentation"):
    ...
    async def do_rtfm(self, ctx, key, obj):
        ...
        e.set_footer(
            text=f"Requested By {ctx.author}", icon_url="https://media.discordapp.net/attachments/1144290431249686528/1144731614258135100/butterfly.gif"
        )
        e.set_thumbnail(url=self.bot.user.display_avatar.url)
        if len(self.matches) == 0:
            return await ctx.send_warning("Could not find anything. Sorry.")
    ...
    @rtfm.command(
        description="Check a tag on discord.py library", usage="[tag]", help="utility"
    )
    async def tag(self, ctx, key: str = None, *, query: str = None):
