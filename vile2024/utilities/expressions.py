...
parameter_parser = lambda params: re.compile(r"--({})(?:\s+([\S\s]*?)(?=\s--|$))?".format(
    "|".join(map(re.escape, params))
))
...
link = re.compile(r"(https?|s?ftp)://(\S+)", re.I)

tag = re.compile(
    r"(?<!\\)(?:\$v)?\{(?P<name>[^:{}]+?)\s*:\s*(?P<value>(?:{[^{}]*}|[^{}]|\}(?=\s*}))*)\}"
)

tiktok_aweme_id = re.compile(
    "https://www.tiktok.com/@.*?/video/(\d+)"
)

youtube_url = re.compile(
    r"(?:http\:|https\:)?\/\/(?:www\.)?youtube\.com\/(?:watch|shorts).*"
)

instagram_url = re.compile(
    r"(?:http\:|https\:)?\/\/(?:www\.)?instagram\.com\/reel.*"
)