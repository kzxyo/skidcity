    ...
    async def ban(self, ctx: commands.Context, option=None, punishment=None):
      ...
      if option.lower() == "set":
       ...
       async with self.bot.db.cursor() as cursor: 
        ...
        await cursor.execute(f"DELETE FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        await ctx.reply(embed=discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: Removed anti ban module"), mention_author=False)
    ...
    async def kick(self, ctx: commands.Context, option=None, punishment=None):
      ...
      if option.lower() == "set":
       ...
       async with self.bot.db.cursor() as cursor: 
        ...
        await cursor.execute(f"DELETE FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        await ctx.reply(embed=discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: Removed anti kick module"), mention_author=False)
