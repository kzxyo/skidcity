...
class mod(commands.Cog):
   ...
   @commands.command(aliases=["yeet", "b", "hackban", "deport"], help="ban a member", usage="[member] <reason>", description="moderation")
   ...
   async def ban(self, ctx, member: Union[discord.Member, discord.User]=None, *, reason=None):
        ...
        if reason:
            reason=f'{reason}'
        if member is None:
            embed = discord.Embed(color=Colors.default, title=f'**,ban**', description='ban a member from the server')
            embed.add_field(name='**uasge**', value=',ban [member]\n,ban [member] [reason]', inline=False)
            embed.add_field(name='**aliases**', value='b, deport, yeet, hackban', inline=False)
   ...
   async def unban(self, ctx, *, user: str=None):
        ...
        if user is None:
            embed = discord.Embed(color=Colors.default, title=f'**,unban**', description='umban a member from the server')
            embed.add_field(name='**uasge**', value=',unban [member]', inline=False)
   ...
   @commands.command(aliases=["k", "boot"], help="ban a member", usage="[member] <reason>", description="moderation")
   @commands.cooldown(1, 10, commands.BucketType.guild)
   @blacklist()
   ...
   async def kick(self, ctx, User: discord.Member=None, *, reason=None):
        ...
        if reason:
            reason=f'{reason}'
        if User is None:
            embed = discord.Embed(color=Colors.default, title=f'**,kick**', description='ban a member from the server')
            embed.add_field(name='**uasge**', value=',kick [member]\n,kick [member] [reason]', inline=False)
            embed.add_field(name='**aliases**', value='k, boot', inline=False)
            return await ctx.reply(embed=embed, mention_author=False)
        async for ban in ctx.guild.bans():
            if ban.user.id == member.id:
                return await ctx.success(f'{member.mention} is already **banned**')
   ...
   async def role(self, ctx, user : discord.Member=None, *, role : discord.Role=None):
    ...
    if role in user.roles:
      ...
      emb = discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: given {role.mention} to {user.mention}") 
