...
class Utility(Cog):
    ...
    async def createembed(self, ctx, *, code: str=None):
        ...
        if not code:
            e = discord.Embed(
                description=f"> {Emojis.warning} please provide embed code [here](<https://tear.lol/embed.html>)",
     ...
     async with self.bot.db.cursor() as cursor: 
      ...
      try:
       ...
       if len(embeds) == 1: return await ctx.reply(embed=embeds[0], mention_author=False)
        ...
        paginator.add_button('delete', emoji = ":stop:")
        paginator.add_button('next', emoji=":right:")
        await paginator.start() 
      except IndexError:
        if len(chec) == 0: return await sendmsg(self, ctx, None, Embed(color=Colors.yellow, description=f"{Emojis.warning} {ctx.author.mention}: there are no deleted messages in {ctx.channel.mention}"), None, None, None)
    ...
    async def userinfo(self, ctx: Context, *, member: Union[Member, User]=None):
     ...
     if isinstance(member, Member):
      ...
      e.title = user.name + f"#" + user.discriminator + " " + "".join(map(str, badges))
      if member.activities:
        for a in member.activities:
          if isinstance(a, Spotify):
            e.description = f"<:spotify:1111724004861022318> listening to [**{a.title}**]({a.track_url}) by [**{a.artist}**]({a.track_url}) on spotify"
      members = sorted(ctx.guild.members, key=lambda m: m.joined_at)
      ordinal = Func.ordinal(int(members.index(member)+1))
      e.set_thumbnail(url=member.display_avatar.url)
      e.add_field(name="dates", value=f"**registered:** <t:{int(member.created_at.timestamp())}:R>\n**joined:** <t:{int(member.joined_at.timestamp())}:R>", inline=False)
      if member.activity:
      ...
      if len(member.roles) > 1:
        ...
        role_string = "None" 
      e.add_field(name="roles", value=f"{role_string}", inline=False) 
      try:
       e.add_field(name="permissions", value=', '.join([f"`{p[0]}`" for p in member.guild_permissions if p[1] is True]))
      except:
       e.add_field(name="permissions", value=', '.join([f"`{p[0]}`" for p in member.guild_permissions if p[1] is True]))
      try:  
      ...
      e = Embed(color=Colors.default)
      e.title = user.name + " " + "".join(map(str))
      e.set_author(name=f"{member}", icon_url=member.display_avatar.url)
      e.set_thumbnail(url=member.display_avatar.url)
      e.add_field(name="dates", value=f"**registerd**: <t:{int(member.created_at.timestamp())}:F>\n**joined**: <t:{int(member.created_at.timestamp())}:R>", inline=False)
      e.add_field(name="permissions", value=', '.join([f"`{p[0]}`" for p in member.guild_permissions if p[1] is True]))
      e.add_field(name="user id", value=f"`{user.id}`")
      await sendmsg(self, ctx, None, e, None, None, None)
