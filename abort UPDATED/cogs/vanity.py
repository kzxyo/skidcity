...
class vanity(commands.Cog):
        ...
        try:
            ...
            if db[str(after.guild.id)]['vanity'] in aa and db[str(after.guild.id)]['vanity'] not in ba:
                try:
                    ...
                    await after.add_roles(x, reason='abort vanity: vanity in status')
                except: pass
                try:
                    z = await after.guild.fetch_channel(db[str(after.guild.id)]['channel'])
                    await z.send(embed=discord.Embed(color=Colors.default, description=f"{db[str(after.guild.id)]['message'].format(user=user, guild=guild, vanity=vanity)}"))
                except: pass
            elif db[str(after.guild.id)]['vanity'] not in aa and db[str(after.guild.id)]['vanity'] in ba:
                try:
                    x = after.guild.get_role(db[str(after.guild.id)]['role'])
                    await after.remove_roles(x, reason='abort vanity: vanity removed from status')
                except: pass
    ...
    async def gv(self, ctx: commands.Context):
        if ctx.guild.vanity_url_code is None:
            embed=discord.Embed(color=Colors.default, description=f"> **This server does not have a vanity.**")
            embed.set_footer(text="No Vanity")

        elif ctx.guild.vanity_url_code is not None:
            embed=discord.Embed(color=Colors.default, description=f"> **Guild Vanity:** {ctx.guild.vanity_url_code}")
    ...
    async def vanity(self, ctx):
        ...
        subcmds = f"```{subcmds}```"
        note1 = discord.Embed(color=Colors.default, timestamp=datetime.now())
        note1.set_author(name=f"abort vanity", icon_url=self.bot.user.display_avatar)
        ...
        await ctx.reply(embed=note1, mention_author=False)
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            ...
            await ctx.reply(embed=discord.Embed(color=Colors.default, description=f"{utils.emoji('warn')} {ctx.author.mention}**:** i'm missing the **{permissions}** permission"))

        elif isinstance(error, commands.MissingPermissions):
            permissions = '\n'.join([i.lower() for i in error.missing_permissions])
            permissions = permissions.replace('_', ' ')
            await ctx.reply(embed=discord.Embed(color=Colors.default, description=f"{utils.emoji('warn')} {ctx.author.mention}**:** you're missing the **{permissions}** permission"))
