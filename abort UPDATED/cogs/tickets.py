...
class closeticket(discord.ui.View):
    ...
    @discord.ui.button(label = "Close", emoji = f"<:lock:1110940615199428649>", style=discord.ButtonStyle.red, custom_id="persistent_view:closeticket")
...
class ticketb(discord.ui.View):
    ...
    @discord.ui.button(label = "Create Ticket", emoji = f"<:x_ticket:1111009092757115012>", style=discord.ButtonStyle.green, custom_id="persistent_view:ticket")
    async def ticketbt(self, interaction: discord.Interaction, button: discord.ui.Button):
      async with interaction.client.db.cursor() as cursor: 
        ...
        if check is not None:
          embed=discord.Embed(description=f"{Emojis.warning} {interaction.user.mention} you already have a ticket made, please close it before making a new one", color=Colors.default)
          ...
          async with interaction.client.db.cursor() as cursor:  
            ...
            await channel.send(f"{interaction.user.mention}", embed=discord.Embed(title = f"<:x_ticket:1111009092757115012> Ticket Created", description = "Thank you for creating a ticket, staff will be right with you shortly.", color = Colors.default), view=closeticket())
    ...
    async def ticket(self, ctx, option=None):
        if (not ctx.author.guild_permissions.administrator):
            ...
            async with self.bot.db.cursor() as cursor:
                ...
                check = await cursor.fetchone()
                if check is not None:
                    em = discord.Embed(color=Colors.default, description=f"{Emojis.warning} {ctx.author.mention} the ticket module has already been setup for this server")
                    await ctx.reply(embed=em, mention_author=False)
                    return
                elif check is None:
                    category = await ctx.guild.create_category("tickets")
                    overwrite = {ctx.guild.default_role: discord.PermissionOverwrite(view_channel=True, send_messages=False)}
                    embed = discord.Embed(title = f"<:x_ticket:1111009092757115012> Tickets <:x_ticket:1111009092757115012>", description = f"To create a ticket, press the <:x_ticket:1111009092757115012> button below this message.", color=Colors.default)
                    ...
                    await self.bot.db.commit()
                    e = discord.Embed(color=Colors.default, description=f"{Emojis.check} {ctx.author.mention} successfully configured ticket module")
            ...
            async with self.bot.db.cursor() as cursor:
              ...
              check = await cursor.fetchone() 
              if check is None:
                em = discord.Embed(color=Colors.default, description=f"{Emojis.warning} {ctx.author.mention} ticket module isn't set")
                await ctx.reply(embed=em, mention_author=False)
                return
                ...
                try:
                  ...
                  await self.bot.db.commit()
                  embed = discord.Embed(color=Colors.default, description=f"{Emojis.check} {ctx.author.mention} ticket module has been disabled")
