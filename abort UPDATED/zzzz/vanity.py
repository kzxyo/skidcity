...
class vanity(commands.Cog):
        ...
        try:
            ...
            if db[str(after.guild.id)]['vanity'] in aa and db[str(after.guild.id)]['vanity'] not in ba:
                ...
                try:
                    ...
                    await z.send(embed=discord.Embed(color=0x495063, description=f"{db[str(after.guild.id)]['message'].format(user=user, guild=guild, vanity=vanity)}"))
                except: pass
    ...
    async def gv(self, ctx: commands.Context):
        if ctx.guild.vanity_url_code is None:
            embed=discord.Embed(color=0x495063, description=f"> **This server does not have a vanity.**")
            embed.set_footer(text="No Vanity")

        elif ctx.guild.vanity_url_code is not None:
            embed=discord.Embed(color=0x495063, description=f"> **Guild Vanity:** {ctx.guild.vanity_url_code}")
    ...
    async def vanity(self, ctx):
        ...
        subcmds = f"```{subcmds}```"
        note1 = discord.Embed(color=0x495063, timestamp=datetime.now())
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            ...
            await ctx.reply(embed=discord.Embed(color=0x495063, description=f"{utils.emoji('warn')} {ctx.author.mention}**:** i'm missing the **{permissions}** permission"))

        elif isinstance(error, commands.MissingPermissions):
            permissions = '\n'.join([i.lower() for i in error.missing_permissions])
            permissions = permissions.replace('_', ' ')
            await ctx.reply(embed=discord.Embed(color=0x495063, description=f"{utils.emoji('warn')} {ctx.author.mention}**:** you're missing the **{permissions}** permission"))
