...
class filters(commands.Cog):
    ...
    async def setsettings(self, guild_id, enabled):
        async with aiosqlite.connect("database.db") as db:
            await db.execute(
                "INSERT OR REPLACE INTO settings(guild_id, enabled) VALUES(?, ?)",
                (guild_id, enabled),
            )
            await db.commit()
    ...
    @commands.command(
        name="musicfiles",
        description="filter music files from being sent in your guild",
    )
    async def musicfiles(self, ctx):
        ...
        await ctx.send(
            f"File extension filtering is currently {status}. Use `{ctx.prefix}fe toggle` to toggle it."
        )
