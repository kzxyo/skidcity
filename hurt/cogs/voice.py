...
from cogs.utilevents import noperms, blacklist
...
class voice(commands.Cog):
    ...
    async def on_ready(self):
        async with self.bot.db.cursor() as cursor:
            await cursor.execute(
                "CREATE TABLE IF NOT EXISTS vmutelock (channel_id INTEGER, mute BOOLEAN);"
            )
            await self.bot.db.commit()

    @commands.command(
        help=f"mass moves users",
        description="utility",
        usage="[channel]",
        aliases=["moveall", "dragall"],
    )
    @blacklist()
        ...
        if origin.members:
            if origin != destination:
                ...
                for member in origin.members:
                    await member.edit(
                        voice_channel=destination, reason=f"Massmoved by {ctx.author}"
                    )
                    moved.append(member)
                    e = Embed(
    ...
    @commands.command(
        help="server mutes a member", description="utility", usage="[user]"
    )
    @blacklist()
    ...
    @commands.command(
        help="unserver mutes a member", description="utility", usage="[user]"
    )
    @blacklist()
    ...
    @commands.command(
        help="unserver deafens a member",
        description="utility",
        usage="[user]",
        aliases=["vudeaf"],
    )
    ...
    @commands.command(
        ...
        description="voice",
        usage="[voicename]",
        aliases=["voicecreate", "voicemake", "makevoice"],
    )
    ...
    async def createvoice(self, ctx, channel_name):
        if channel_name is None:
            ...
            embed.add_field(
                name="commands", value=",createvoice\n,deletevoice", inline=False
            )
            embed.add_field(
                name="usage", value=f"```,createvoice .gg/runs```", inline=False
            )
            embed.add_field(
                name=f"aliases",
                value="voicecreate, voicemake, makevoice\nvoicedelete, removevoice, voiceremove",
            )
        ...
        if not echnl:
            ...
            e = Embed(
                description=f"{Emojis.check} The **{channel_name}** voice channel has been created."
            )
    ...
    @commands.command(
        help="delete voice channel",
        description="voice",
        usage="[channel]",
        aliases=["voicedelete", "removevoice", "voiceremove"],
    )
    @commands.cooldown(1, 4, commands.BucketType.user)
    @blacklist()
    async def deletevoice(self, ctx, channel_name):
        ...
        if voice_channel:
            ...
            await ctx.message.delete()
            e = Embed(
                description=f"{Emojis.check} The **{channel_name}** voice channel has been deleted."
            )
            await ctx.send(embed=e)
        else:
            e = Embed(
                description=f"{Emojis.warn} The voice channel **{channel_name}** does not exist."
            )
            await ctx.reply(embed=e, mention_author=False)

    @commands.command(
        help="mute all members in a voice channel", description="voice", usage="[NONE]"
    )
    @commands.cooldown(1, 4, commands.BucketType.user)
    ...
    @commands.command(
        help="unmutes all members in a voice channel",
        description="voice",
        usage="[NONE]",
    )
    @commands.cooldown(1, 4, commands.BucketType.user)
    ...
    @commands.command(
        help="deafens all members in a voice channel",
        description="voice",
        usage="[NONE]",
        aliases=["vdeafall"],
    )
    ...
    @commands.command(
        help="undeafens all members in a voice channel",
        description="voice",
        usage="[NONE]",
        aliases=["vudeafall"],
    )
    ...
    @commands.command(
        help="disconnects all members in a voice channel",
        description="voice",
        usage="[NONE]",
        aliases=["vdcall"],
    )
