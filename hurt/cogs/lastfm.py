...
class lastfm(commands.Cog):
    ...
    async def on_ready(self):
        async with self.bot.db.cursor() as cursor:
            await cursor.execute(
                "CREATE TABLE IF NOT EXISTS lastfm (user_id INTEGER, username TEXT, customreact TEXT);"
            )
        await self.bot.db.commit()

    @commands.group(
        description="lastfm",
        help="Get your spotify statistics through lastfm.",
        usage="[NONE]",
        aliases=["fm", "lf"],
    )
    @blacklist()
    ...
    async def set(self, ctx, ref=None):
        ...
        if not await fmuser.userexists(ref):
            embed = discord.Embed(
                description=f"{Emojis.warn} Invalid Last.fm username", color=0x2F3136
            )
            ...
            async with self.bot.db.cursor() as cursor:
                ...
                if check is None:
                    await cursor.execute(
                        "INSERT INTO lastfm VALUES (?, ?, ?)", (ctx.author.id, ref)
                    )
                ...
                embed = discord.Embed(
                    description=f" {ctx.message.author.mention}: Your **Last.fm** username has been set to **{ref}**",
                    color=0x2F3136,
                )
    ...
    @lastfm.command(
        description="lastfm",
        help="remove your lastfm account",
        usage="[name]",
        aliases=["disconnect", "unlink" "unset"],
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    @blacklist()
        ...
        async with self.bot.db.cursor() as cursor:
            ...
            if check is None:
                return await ctx.reply(
                    discord.Embed(
                        description=f"{Emojis.warn} set a **[lastfm](https://last.fm) account** linked",
                        color=0x2F3136,
                    )
                )
            await cursor.execute(
            ...
            await ctx.reply(
                discord.Embed(
                    color=0x2F3136,
                    description=f"{Emojis.check} your **[lastfm](https://last.fm) account** has been removed",
                )
            )

    @lastfm.command(
        description="see what you are listening to",
        help="nowplaying lastfm command",
        usage="[user]",
        aliases=["np"],
    )
            ...
            async with self.bot.db.cursor() as cursor:
                ...
                if check is not None:
                    ...
                    if user != "error":
                        ...
                        artist = crime["recenttracks"]["track"][0]["artist"][
                            "name"
                        ].replace(" ", "+")
                        image = crime["recenttracks"]["track"][0]["image"][3]["#text"]
                        album = (
                            crime["recenttracks"]["track"][0]["album"]["#text"]
                            or "Not an album."
                        )
                    ...
                    embed = discord.Embed(
                        description=f"<:crimelastfm:1082118965041577984> **{member}** doesn't have a **Last.fm account** linked. Use `;lf set <username>` to link your **account**.",
                        color=0x2F3136,
                    )
                    await ctx.send(embed=em)
        except Exception as e:
            embed = discord.Embed(
                description=f"{Emojis.warn} {ctx.author.mention}: unable to get **{member.name}'s** recent track ",
                color=0x2F3136,
            )
            print(e)
    ...
    @lastfm.command(
        description="have custom reactions for your lastfm nowplaying embed",
        help="custom lastfm reaction",
        usage="[emojis]",
        aliases=["customreactions", "customreaction"],
    )
    @commands.cooldown(1, 5, commands.BucketType.user)
    @blacklist()
