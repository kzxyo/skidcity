...
class roles(commands.Cog):
    ...
    async def role(self, ctx):
        embed = discord.Embed(
            color=Colors.default, title="roles", description="manage roles with crime"
        )
        embed.add_field(
            name="usage", value="```role [subcommand] [target] [other]```", inline=False
        )
        embed.add_field(
            name="commands",
            value="> role create - creates a role\n> role delete - deletes a role\n> role give - gives a user a role\n> role take - takes a role from a user\n> role rename - renames a role\n> role info - gets info on a role",
            inline=False,
        )
