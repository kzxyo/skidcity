...
class user(Cog):
    ...
    @command(
        help="creates category",
        description="utility",
        usage="[categoryname]",
        aliases=["categorycreate", "makecategory", "categorymake"],
    )
    @blacklist()
    ...
    @command(
        help="deletes category",
        description="utility",
        usage="[categoryname]",
        aliases=["categorydelete", "removecategory", "removecreate"],
    )
    @blacklist()
    ...
    @command(
        ...
        description="utility",
        usage="[channelname]",
        aliases=["channelcreate", "makechannel", "channelmake"],
    )
    ...
    async def createchannel(self, ctx, channel_name: str):
        if channel_name is None:
            embed = discord.Embed(
                color=0xF7F9F8,
                title="text channel",
                description="create or delete a text channel",
            )
            embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.avatar.url)
            embed.add_field(name="category", value="utility")
            embed.add_field(
                name="commands", value=",createchannel\n,deletechannel", line=False
            )
            embed.add_field(
                name="usage", value=f"```,createchannel .gg/runs```", inline=False
            )
            embed.add_field(
                name=f"aliases",
                value="channelcreate, makechannel, channelmake\nchanneldelete, removechannel, channelremove",
            )
        ...
        if not echnl:
            ...
            e = Embed(
                description=f"{Emojis.check} The **{channel_name}** channel has been created."
            )
    ...
    @command(
        ...
        description="utility",
        usage="[channel]",
        aliases=["channeldelete", "removechannel", "channelremove"],
    )
    @cooldown(1, 4, BucketType.user)
    @blacklist()
    async def deletechannel(self, ctx, channel: discord.TextChannel):
        ...
        if echnl:
            ...
            await ctx.message.delete()
            e = Embed(
                description=f"{Emojis.check} The **{channel}** channel has been deleted."
            )
            await ctx.send(embed=e)
