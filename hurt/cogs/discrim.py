...
class tracker(commands.Cog):
    ...
    async def on_ready(self):
        async with self.bot.db.cursor() as cursor:
            await cursor.execute(
                "CREATE TABLE IF NOT EXISTS tracker (guild_id INTEGER, channel INTEGER)"
            )
            ...
            if before.discriminator == "0001":
                ...
                for x in self.bot.guilds:
                    async with self.bot.db.cursor() as e:
                        await e.execute(
                            f"SELECT * FROM tracker WHERE guild_id = {x.id}"
                        )
    ...
    @tracker.command(name="add", description="Adds a channel to track #0001 tags")
    @commands.has_permissions(manage_channels=True)
    @blacklist()
    async def add(self, ctx, channel=None):
        ...
        try:
            ...
            async with self.bot.db.cursor() as c:
                await c.execute(
                    f"SELECT * FROM tracker WHERE guild_id = ? AND channel = ?",
                    (
                        ctx.guild.id,
                        channel,
                    ),
                ...
                if check is not None:
                    await c.execute(
                        "UPDATE tracker SET guild_id = ? AND channel = ?",
                        (
                            ctx.guild.id,
                            channel,
                        ),
        ...
        async with ctx.typing():
            ...
            if available_tags:
                ...
                for group in tags:
                    ...
                    page.description = "\n".join(
                        [
                            f"`{idx+1+i}` **-** {x['user']}: {discord.utils.format_dt(x['time'], style='R')}"
