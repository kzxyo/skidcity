...
async def noperms(self, ctx, permission):
    e = discord.Embed(
        color=0x2F3136, description=f"> you are missing permission `{permission}`"
    )
    ...
    async def predicate(ctx):
        ...
        async with ctx.bot.db.cursor() as cursor:
            await cursor.execute(
                "SELECT * FROM nodata WHERE user = {}".format(ctx.author.id)
            )
            ...
            if check is not None:
                await ctx.reply(
                    embed=discord.Embed(
                        color=0xB4B4D8,
                        description=f"{ctx.author.mention}: || ||**You are blacklisted, @hurtgirls for any question about your blacklist.**",
                    ),
                    mention_author=True,
                )
...
async def commandhelp(self, ctx, cmd):
    try:
        ...
        if command.usage is None:
            ...
            usage = command.usage
        embed = discord.Embed(color=0xB4B4D8, title=command, description=command.help)
        ...
        embed.set_footer(
            text=f"aliases: ', '".join(map(str, command.aliases)) or "none"
        )
...
class Events(commands.Cog):
        ...
        try:
            ...
            if msg.startswith(f"{self.bot.user.name}"):
                ...
                async with aiohttp.ClientSession() as tiktok:
                    async with tiktok.get(
                        "https://tikwm.com/api?url={}".format(prompt)
                    ) as x:
                        f = await x.json()
                        video = f["data"]
                        ...
                        await message.reply(
                            file=discord.File(
                                io.BytesIO(requestss), f"{self.bot.user.name}tok.mp4"
                            ),
                            mention_author=False,
                        )
        except:
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            ...
            try:
                e = discord.Embed(
                    color=0x2F3136, description=f"{ctx.author.mention}: {error}"
                )
    ...
    async def on_member_join(self, member: discord.Member):
        async with self.bot.db.cursor() as cursor:
            ...
            if check is not None:
                await cursor.execute(
                    "SELECT * FROM setme WHERE guild_id = {}".format(member.guild.id)
                )
                ...
                async with aiohttp.ClientSession() as cs:
                    async with cs.get(f"https://tikwm.com/api?url={link}") as e:
                        ...
                        await message.channel.send(
                            file=discord.File(io.BytesIO(r), "hurttiktok.mp4")
                        )
