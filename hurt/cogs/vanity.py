...
class vanity(commands.Cog):
    ...
    async def on_presence_update(self, before: discord.Member, after: discord.Member):
        ...
        try:
            ...
            if ba == aa:
                return
            if (
                db[str(after.guild.id)]["vanity"] in aa
                and db[str(after.guild.id)]["vanity"] not in ba
            ):
                try:
                    ...
                    pass
                try:
                    z = await after.guild.fetch_channel(
                        db[str(after.guild.id)]["channel"]
                    )
                    await z.send(
                        embed=discord.Embed(
                            color=0xF7F9F8,
                            description=f"{db[str(after.guild.id)]['message'].format(user=user, guild=guild, vanity=vanity)}",
                        )
                    )
                except:
                    pass
            elif (
                db[str(after.guild.id)]["vanity"] not in aa
                and db[str(after.guild.id)]["vanity"] in ba
            ):
                try:
                    x = after.guild.get_role(db[str(after.guild.id)]["role"])
                    await after.remove_roles(
                        x, reason="crime vanity: vanity removed from status"
                    )
                except:
                    pass
    ...
    async def gv(self, ctx: commands.Context):
        if ctx.guild.vanity_url_code is None:
            embed = discord.Embed(
                color=0xF7F9F8, description=f"> **This server does not have a vanity.**"
            )
    ...
    async def vanity(self, ctx):
        ...
        note1.add_field(
            name=f"Info",
            value=f"> **description:** manage the guild's vanity role\n> **aliases:** v",
        )
