    ...
    class Paginator(discord.ui.View):
        def __init__(
            ...
            *,
            ...
            self.previous_button = previous_button or discord.ui.Button(
                emoji="<:icons_rightarrow:1135806059366322186>",
                style=discord.ButtonStyle.grey,
            )
            self.next_button = next_button or discord.ui.Button(
                emoji="<:icons_rightarrow:1135806059366322186>",
                style=discord.ButtonStyle.grey,
            )
        ...
        async def start(
            self, ctx: commands.Context, pages: list[discord.Embed], reply: bool = True
        ):
            self.pages = pages
            ...
            self.page_counter = Views.PaginatorPageCounter(
                self.page_counter_style, len(self.pages), self.current_page
            )
            ...
            if reply:
                self.message = await ctx.reply(
                    embed=self.pages[self.initial_page], view=self, mention_author=False
                )
            else:
                self.message = await ctx.send(
                    embed=self.pages[self.initial_page], view=self
                )
