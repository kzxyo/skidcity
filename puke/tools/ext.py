...
class Client(object): 
  ...
  async def send_success(self, ctx: Union[commands.Context, discord.Interaction], message: str, ephemeral: bool=True) -> discord.Message or None: 
   if isinstance(ctx, commands.Context): return await ctx.reply(embed=discord.Embed(color=self.bot.color, description=f">>> {ctx.author.mention}: {message}"))
   else: return await ctx.response.send_message(embed=discord.Embed(color=self.bot.color, description=f">>> {ctx.user.mention}: {message}"), ephemeral=ephemeral)
